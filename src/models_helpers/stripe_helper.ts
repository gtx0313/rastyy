...
export async function getStripeCustomerId({ userId, email, isStripeKeyLive }: CustomerId) {
  try {
    ...
    if (isStripeKeyLive) await firestoreAdmin.collection('users').doc(userId).update({ stripeCustomerIdLive: newCustomer.id });
    if (!isStripeKeyLive) await firestoreAdmin.collection('users').doc(userId).update({ stripeCustomerIdTest: newCustomer.id });

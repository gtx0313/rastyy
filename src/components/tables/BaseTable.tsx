...
export function BaseTable({ columns, data }: IBaseTableProps) {
  ...
  return (
    <>
      <div className='shadow-md rounded-lg border border-slate-300 border-opacity-30'>
          ...
          <table className='min-w-[1000px] w-full border-transparent'>
            <thead className=''>
              {instance.getHeaderGroups().map((headerGroup: HeaderGroup<any>) => (
                <tr className='m-0 p-0 border-none' key={headerGroup.id}>
            ...
            <tbody>
              {instance.getRowModel().rows.map((row: Row<any>) => (
                <tr key={row.id} className='hover:bg-slate-50 hover transition hover:dark:bg-dark-400'>
                  {row.getVisibleCells().map((cell) => (
                    <td className='text-left text-[15px] py-4 px-2 border-b border-gray-100 dark:border-gray-700' key={cell.id}>
